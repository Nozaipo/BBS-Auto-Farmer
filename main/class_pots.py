...
with open("_saga quest farmed.json", "r") as file:
...
def show_pot(json_content):
    ...
    with open("pots.json", "w") as json_file:
        json.dump(info_pots_json, json_file)
        ...
        if tickets >= 10:
            if "5001" in str(lastBagId):
                if amountBag < 7100:
                    ...
                    with open("_saga quest farmed.json", "w") as json_file:
                        json.dump(_saga_quest_farmed, json_file)
...
def show_bags(json_content):
    ...
    with open("bags.json", "w") as json_file:
        json.dump(info_bag_json, json_file)
